...
def get_df_from_csv_string(csv_string):
    with StringIO(csv_string) as csv_stream:
        return pd.read_csv(csv_stream, index_col="ID", sep="\t", dtype=str, quoting=csv.QUOTE_ALL, quotechar='"')

def get_df_from_file(file_name):
    return pd.read_csv(file_name, index_col="ID", sep="\t", dtype=str, quoting=csv.QUOTE_ALL, quotechar='"')
