...
def get_batch_source(source_type, source_name):
    if source_type == "file-system":
        return FileSystemBatchSource(source_name)
    elif source_type == "s3":
        source_names = source_name.split("/")
        return S3BatchSource(bucket_name=source_names[0], folder_name=source_names[1])
...
def main():
    ...
    parser.add_argument("--source-type", choices=["file-system", "s3", "sqs"], required=True)
    parser.add_argument("--source-name", required=True, help="If using S3, use the format bucket-name/folder-name.")
    parser.add_argument("--dest-type", choices=["file-system", "s3", "sqs"], required=True)
    ...
    if args.populate:
        ...
        batch_source = get_batch_source(args.source_type, args.source_name)
